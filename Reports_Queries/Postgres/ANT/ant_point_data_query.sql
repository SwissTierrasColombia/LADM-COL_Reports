...
t AS (
	SELECT t_id, ST_ForceRHR(geometria) AS geometria FROM rubi.lc_terreno WHERE t_id = (SELECT terreno_t_id FROM parametros)
),
...
t_simple AS (
	SELECT t_id, (ST_Dump(geometria)).path[1] AS parte, ST_ForceRHR((ST_Dump(geometria)).geom) AS geom FROM rubi.lc_terreno WHERE t_id = (SELECT terreno_t_id FROM parametros)
),
...
linderos AS (
	SELECT lc_lindero.t_id, lc_lindero.geometria AS geom  FROM rubi.lc_lindero JOIN rubi.col_masccl ON col_masccl.ue_mas_lc_terreno = (SELECT terreno_t_id FROM parametros) AND lc_lindero.t_id = col_masccl.ccl_mas
),
...
terrenos_asociados_linderos AS (
	SELECT DISTINCT col_masccl.ue_mas_lc_terreno AS t_id_terreno, col_masccl.ccl_mas AS t_id_lindero FROM rubi.col_masccl WHERE col_masccl.ccl_mas IN (SELECT t_id FROM linderos) AND col_masccl.ue_mas_lc_terreno != (SELECT terreno_t_id FROM parametros)
),
-- Puntos linderos asociados al terreno
puntos_lindero AS (
	SELECT DISTINCT lc_puntolindero.t_id, lc_puntolindero.geometria AS geom FROM rubi.lc_puntolindero JOIN rubi.col_puntoccl ON col_puntoccl.ccl IN (SELECT t_id FROM linderos) AND lc_puntolindero.t_id = col_puntoccl.punto_lc_puntolindero
),
