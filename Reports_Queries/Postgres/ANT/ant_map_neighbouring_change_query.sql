WITH
                    parametros AS (
                        SELECT
	                        ...
	                        SELECT t_id, ST_ForceRHR(geometria) as geometria FROM ladm_lev_cat_v1.lc_terreno AS t, parametros where t.t_id = 1522 --parametrizar schema, tabla, atributos y WHERE
                                ),
                            --bordes de la extension del poligono
                            a AS (
	                        SELECT ST_SetSRID(ST_MakePoint(st_xmin(t.geometria), st_ymax(t.geometria)), ST_SRID(t.geometria)) AS p FROM t --parametrizar geometria
                            ),
                            b AS (
                                SELECT ST_SetSRID(ST_MakePoint(st_xmax(t.geometria), st_ymax(t.geometria)), ST_SRID(t.geometria)) AS p FROM t --parametrizar geometria
                            ),
                            c AS (
                                SELECT ST_SetSRID(ST_MakePoint(st_xmax(t.geometria), st_ymin(t.geometria)), ST_SRID(t.geometria)) AS p FROM t --parametrizar geometria
                            ),
                            d AS (
                                SELECT ST_SetSRID(ST_MakePoint(st_xmin(t.geometria), st_ymin(t.geometria)), ST_SRID(t.geometria)) AS p FROM t --parametrizar geometria
                            ),
                            ...
                            SELECT
                                ...
                                ( SELECT ST_SetSRID(ST_MakePoint(st_x(ST_centroid(t.geometria)), st_y(ST_centroid(t.geometria))), ST_SRID(t.geometria)) AS p FROM t ) --parametrizar geometria
                                WHEN criterio_observador = 2 THEN --Centro del extent
                                ( SELECT ST_SetSRID(ST_MakePoint(st_x(ST_centroid(st_envelope(t.geometria))), st_y(ST_centroid(st_envelope(t.geometria)))), ST_SRID(t.geometria)) AS p FROM t ) --parametrizar geometria
                                WHEN criterio_observador = 3 THEN --Punto en la superficie
                                ( SELECT ST_SetSRID(ST_PointOnSurface(geometria), ST_SRID(t.geometria)) AS p FROM t ) --parametrizar geometria
                                WHEN criterio_observador = 4 THEN --Punto mas cercano al centroide pero que se intersecte el poligono si esta fuera
                                ( SELECT ST_SetSRID(ST_MakePoint(st_x( ST_ClosestPoint( geometria, ST_centroid(t.geometria))), st_y( ST_ClosestPoint( geometria,ST_centroid(t.geometria)))), ST_SRID(t.geometria)) AS p FROM t ) --parametrizar geometria
                            ...
                            norte AS (
                                SELECT ST_SetSRID(ST_MakePolygon(ST_MakeLine(ARRAY [a.p, b.p, m.p, a.p])), ST_SRID(t.geometria)) geom FROM t,a,b,m --parametrizar geometria
                            ),
                            este AS (
                                SELECT ST_SetSRID(ST_MakePolygon(ST_MakeLine(ARRAY [m.p, b.p, c.p, m.p])), ST_SRID(t.geometria)) geom FROM t,b,c,m --parametrizar geometria
                            ),
                            sur AS (
                                SELECT ST_SetSRID(ST_MakePolygon(ST_MakeLine(ARRAY [m.p, c.p, d.p, m.p])), ST_SRID(t.geometria)) geom FROM t,m,c,d --parametrizar geometria
                            ),
                            oeste AS (
                                SELECT ST_SetSRID(ST_MakePolygon(ST_MakeLine(ARRAY [a.p, m.p, d.p, a.p])), ST_SRID(t.geometria)) geom FROM t,a,m,d --parametrizar geometria
                            )
                            ,limite_poligono as(
                                SELECT t_id, ST_Boundary(geometria) geom FROM t --parametrizar atributos
                            )
                            ,limite_vecinos as (  --obtiene el limite de los terrenos colindantes, filtrados por bounding box
                                select o.t_id, ST_Boundary(o.geometria) geom from t, ladm_lev_cat_v1.lc_terreno o where o.geometria && st_envelope(t.geometria) and t.t_id <> o.t_id --parametrizar schema, tabla y atributos
                            )
                            ...
                            , punto_nw as (
                                ...
                                FROM 	puntos_terreno,
                                    (SELECT ST_SetSRID(ST_MakePoint(st_xmin(st_envelope(geometria)), st_ymax(st_envelope(geometria))), ST_SRID(geometria)) as nw FROM t ) a --parametrizar geometria
                            ...
                            , puntos_ordenados as (
                                SELECT case when id-m+1 <= 0 then total + id-m else id-m+1 end as id, geom , st_x(geom) x, st_y(geom) y FROM
                                    ...
                                    SELECT row_number() OVER (ORDER BY path) AS id
                                        ...
                                        ,total
                                    FROM (
                                        SELECT (ST_DumpPoints(ST_ForceRHR(geometria))).* AS dp --parametrizar geometria
                                            ,ST_NPoints(geometria) total --parametrizar geometria
                                            ,geometria
                                        FROM t
                                        ) AS a
                                        ,(
                            ...
                            ,ST_AsGeoJSON(lineas_colindancia.geom)::json AS geometry FROM lineas_colindancia) as f) as ff