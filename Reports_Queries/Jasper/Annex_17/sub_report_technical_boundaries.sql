...
SELECT
  ...
  ,(SELECT array_to_string( array_agg(puntos_lindero_ordenados.id || ': N=' || round(st_y(puntos_lindero_ordenados.geom)::numeric,2) || ', E=' || round(st_x(puntos_lindero_ordenados.geom)::numeric,2) ), '; ') FROM puntos_lindero_ordenados WHERE st_intersects(colindantes.geom,puntos_lindero_ordenados.geom) AND puntos_lindero_ordenados.id NOT IN (desde, hasta) ) AS nodos
