...
(
	SELECT c1.parte, c1.id AS "from", CASE WHEN c1.id + 1 <= c1.punto_final THEN c1.id + 1 ELSE c1.punto_final - c1.id + c1.punto_inicial  END AS "to", trunc(c1.x,2) as x, trunc(c1.y,2) as y, trunc(st_distance(c1.geom, c2.geom)::numeric, CASE WHEN 'ZONA_URBANA' = 'ZONA_RURAL'  THEN 2 ELSE 1 END) AS dist, c1.geom
